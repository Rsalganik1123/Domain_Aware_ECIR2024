...
import torch
import pandas as pd
from src2.benchmarks.ScoreReg.model_graph import NGCF, LightGCN
...
def metrics_graph_bpr(model, test_data, top_k, sid_pop_total, user_num):
	...
	print("predictions", test_data['pred'])
	...
	try: 
		...
		print('exception:{}\n neg_score_table:{}\n, test_data_neg:{}\n'.format(e, neg_scores_table_alluser,  test_data_neg)) 
		exit() 
	pos_score_final = pd.DataFrame(test_data_pos.pred.values) # test pos item num x 1
	neg_score_final = pd.DataFrame(neg_scores_table_testuser.values) # test pos item num x 100
...
# def make_recs(model_path, output_path): 
# 	data_path = '/home/mila/r/rebecca.salganik/Projects/MusicSAGE/src2/benchmarks/ScoreReg'
# 	dataset='mus2'
# 	user_num, item_num, train_data_len = load_all_custom(test_num=100, dataset=dataset)
# 	plain_adj = sp.load_npz(f'{data_path}/data/final_{dataset}/s_adj_mat.npz')
# 	norm_adj = sp.load_npz(f'{data_path}/data/final_{dataset}/s_norm_adj_mat.npz')
# 	mean_adj = sp.load_npz(f'{data_path}/data/final_{dataset}/s_mean_adj_mat.npz')
# 	model = LightGCN(user_num, item_num, norm_adj).cuda()
# 	model = torch.load(model_path)
# 	print(model)
# 	print(model.embedding_dict.item_emb)
